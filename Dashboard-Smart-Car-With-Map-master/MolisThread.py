...
DataSensor = [[0,1],                                                  # nilai Array untuk sensor hujan (0=hujan, 1=terang)
              ...
              [10000,15000,20000,25000]]                             # nilai Array untuk sensor Deteksi Jarak Lampu Lalu Lintas  (satuan cm)
...
def kontrolJarak(sensor_depan, sensor_kiri, sensor_kanan, sensor_depan_kanan, sensor_depan_kiri):
    ...
    if sensor_depan in range(0, 500):
        pesan_aksi_sensor_jarak = 'Jarak depan terlalu dekat. Rem hingga jarak 500 cm'
        if kecepatan in range(30, 60):
            kurangiKecepatan(20)

    if sensor_kiri in range(0, 15):
        pesan_aksi_sensor_jarak = 'Jarak kiri terlalu dekat. Geser ke kanan'

    if sensor_kanan in range(0, 15):
        pesan_aksi_sensor_jarak = 'Jarak kanan terlalu dekat. Geser ke kiri'

    if sensor_depan_kanan in range(0, 200):
        pesan_aksi_sensor_jarak = 'Jarak depan kanan terlalu dekat. Belok serong kiri'

    if sensor_depan_kiri in range(0, 200):
        pesan_aksi_sensor_jarak = 'Jarak depan kiri terlalu dekat. Belok serong kanan'
